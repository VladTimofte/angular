...
import { documentExpired } from 'src/app/utils/booleans';
...
export class AddEditAllocationModalComponent {
  ...
  keepAllocationUpdated(value: Allocation) {
    this.allocation = {
      ...this.allocation,
      ...value,
      startDate: moment(this.modalForm.value.startDate).valueOf(),
      endDate: moment(this.modalForm.value.endDate).valueOf(),
    };
  }
